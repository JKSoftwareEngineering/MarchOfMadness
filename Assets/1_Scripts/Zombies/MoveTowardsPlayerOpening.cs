...
public class MoveTowardsPlayerOpening : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (thisZombie.zombie.isAlive())
        {
            ...
            {
                float temp = Vector3.Distance(transform.position, Survivors[i].transform.position);
                if (temp < furthest)
                {
                    furthest = temp;
                    target = i;
            ...
            transform.LookAt(Survivors[target].transform.position);
            ...
            if (!HitTimer)
            {
                //if (Vector3.Distance(player.transform.position, transform.position) < 40f) <--- this is the only difference from the original
                //{
                    cont.Move(transform.forward * thisZombie.zombie.getSpeed() * Time.deltaTime);
                //}
            }
        ...
        if(Vector3.Distance(Survivors[target].transform.position, transform.position) < 1.5f)
        {
            if (!Attack)
            {
